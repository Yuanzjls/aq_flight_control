...
`+` Add ability to report current (power) sensor data via Mavlink (at this point using a current sensor directly with AQ still requires custom code or the un-released 鈥淧DB鈥?expander.)  
`+` Add more concise system status reporting in Mavlink 鈥渃ustom mode鈥?variable, and for other upcoming telemetry options.  
...
##### 7.1.1852 -�?EXPR 鈥?Feb. 17, 2015

`~` New version numbering format now excludes the SVN revision (鈥渞鈥? number.  New format is  Major.Minor.Build 鈥?Label.  

...
//ALIGN(4)
rt_uint32_t *radioTaskStack;//[RADIO_STACK_SIZE] __attribute__((section(".ccm")));
static struct rt_thread radioTask;
...
//                AQ_PRINTF("CYRF6936 on RC port %d\n", i);
    ...
    radioTaskStack = aqStackInit(RADIO_STACK_SIZE, "RADIO");//kevin

    //radioData.radioTask = CoCreateTask(radioTaskCode, (void *)0, RADIO_PRIORITY, &radioTaskStack[RADIO_STACK_SIZE-1], RADIO_STACK_SIZE);
   
     rt_thread_init(&radioTask,
                            "radio",
                            radioTaskCode,
                            RT_NULL,
                            (rt_uint8_t*)radioTaskStack,
                            RADIO_STACK_SIZE*4,
                            RADIO_PRIORITY,
                            5);
 
    rt_thread_startup(&radioTask);
}

...
//       }
        ...
        if (supervisorData.state & STATE_ARMED) {
...
#ifdef _DISABLE_MANUAL
            //vertStick = RADIO_THROT - RADIO_MID_THROTTLE;//kevin

            /*
            //这里或许有不合适的地方
            if ((navData.mode >= NAV_STATUS_ALTHOLD)&&(navData.mode < NAV_STATUS_MISSION))//kevin
            {
                 if (vertStick > p[CTRL_DBAND_THRO])//kevin
                    take_off = true;//kevin
            }
            ...
            {
                 take_off = false;//kevin
            }
            */
            //这里或许有不合适的地方
            if (navData.mode >= NAV_STATUS_ALTHOLD)//kevin
            {
                if (navData.holdSpeedAlt/*targetHoldSpeedAlt*/>0.2f)
                {
                    navData.takeOff = true;//kevin
                }
            }
            else
            {
                navData.takeOff = false;//kevin
            }
#endif
            ...
            if (RADIO_THROT > p[CTRL_MIN_THROT] || navData.mode >= NAV_STATUS_ALTHOLD) {
                ...
                ratesActual[RPY_R] = IMU_DRATEX + UKF_GYO_BIAS_X;//kevin
...
#ifdef _DISABLE_MANUAL
                if ((motorsData.throttle == 0)||((navData.mode >= NAV_STATUS_ALTHOLD)&&(navData.takeOff == false)))//kevin
#else
...
#ifdef _DISABLE_MANUAL
                        if (navData.takeOff == false)//kevin
                        {
...
#ifdef _DISABLE_MANUAL
                            if (((navData.mode >= NAV_STATUS_ALTHOLD)&&(navData.takeOff == false)))//kevin
                            {
...
void controlInit(void) {
    ...
    //controlData.controlTask = CoCreateTask(controlTaskCode, (void *)0, CONTROL_PRIORITY, &controlTaskStack[CONTROL_STACK_SIZE-1], CONTROL_STACK_SIZE);//kevin

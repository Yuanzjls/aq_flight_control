...
rt_uint32_t *commTaskStack;
struct rt_thread  commTask;
...
void commNotice(const char *s) {
    ...
    if (commData.noticeQueueInit)
        rt_mb_send(&commData.notices, (rt_uint32_t )s);//CoPostQueueMail(commData.notices, (void *)s);//kevin
}
...
uint8_t commAvailable(commRcvrStruct_t *r) {
    ...
    switch (commData.portTypes[port]) {
        ...
        case COMM_PORT_TYPE_CAN:
//            if (commData.portHandles[port])
//                return canUartAvailable(commData.portHandles[port]);//kevin
            break;
...
commTxBuf_t *commGetTxBuf(uint8_t streamType, uint16_t maxSize) {
    ...
    if (commData.typesUsed & streamType) {
        ...
        rt_mutex_take(&commData.txBufferMutex, RT_WAITING_FOREVER);//CoEnterMutexSection(commData.txBufferMutex);//kevin
        ...
        rt_mutex_release(&commData.txBufferMutex);//CoLeaveMutexSection(commData.txBufferMutex);//kevin
...
//                    commData.txStackTails[port] = (tail + 1) % COMM_STACK_DEPTH;//kevin
//                }
                break;
        }
}
...
static void commCheckNotices(void) {
    ...
    if(rt_mb_recv(&commData.notices,(rt_uint32_t*)&s, RT_WAITING_NO)== RT_EOK)//if (s)//kevin
		{
        int i;
#ifdef COMM_LOG_FNAME
        // write to disk
...
static void commCheckTelem(void) {//kevin
    int i;

    if (rt_sem_take(&runData.runFlag,0) == RT_EOK)
        for (i = 0; i < COMM_MAX_CONSUMERS; i++)
...
void commTaskCode(void *unused) {
    ...
    while (1) {
        ...
        commCheckNotices();
        commCheckTelem();//kevin
//        canCheckMessage(loops);//kevin
        commCheckRcvr();
        //canUartStream();//kevin
...
static char mb_pool[COMM_NOTICE_DEPTH*2];
void commNoticesInit(void) {

    memset((void *)&commData, 0, sizeof(commData));

    rt_mb_init(&commData.notices,
                        "mbt0",            /* Ãû³ÆÊÇmbt */
                        &mb_pool[0],       /* ÓÊÏäÓÃµ½µÄÄÚ´æ³ØÊÇmb_pool */
                        sizeof(mb_pool)/4, /* ÓÊÏäÖÐµÄÓÊ¼þÊýÄ¿£¬ÒòÎªÒ»·âÓÊ¼þÕ¼4×Ö½Ú */
                        RT_IPC_FLAG_FIFO); /* ²ÉÓÃFIFO·½Ê½½øÐÐÏß³ÌµÈ´ý */
    // notice queue
    //commData.notices = CoCreateQueue(commData.noticeQueue, COMM_NOTICE_DEPTH, EVENT_SORT_TYPE_FIFO);
    //if (commData.notices != E_CREATE_FAIL)//kevin
...
void commInit(void) {
    ...
    // setup mutex
    rt_mutex_init(&commData.txBufferMutex, "smutex1", RT_IPC_FLAG_FIFO);//CoCreateMutex();//kevin

    commTaskStack = aqStackInit(COMM_STACK_SIZE, "COMM");

     //= CoCreateTask(commTaskCode, (void *)0, 5, &commTaskStack[COMM_STACK_SIZE-1], COMM_STACK_SIZE);//kevin

     rt_thread_init(&commTask,
                            "comm",
                            commTaskCode,
                            RT_NULL,
                            (rt_uint8_t*)commTaskStack,
                            COMM_STACK_SIZE*4,
                            5,
                            5);
 
    rt_thread_startup(&commTask);

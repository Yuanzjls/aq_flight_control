...
void runTaskCode(void *unused) {
    ...
    while (1) {
        // wait for data
        rt_sem_take(imuData.sensorFlag, RT_WAITING_FOREVER);//CoWaitForSingleFlag(imuData.sensorFlag, 0);//kevin



        // soft start GPS accuracy
        runData.accMask *= 0.9f;//kevin  0.999f
        ...
        {   //kevin
            ...
            //CoClearFlag(gpsTaskData.gpsPosFlag);//kevin
            ...
            //CoClearFlag(gpsTaskData.gpsVelFlag);//kevin
        }
        ...
        // runData.alt_b = ALT_POS - alt_delta;
#endif
        rt_sem_release(&runData.runFlag);
...
void runInit(void) {
    ...
    //runData.runFlag = CoCreateFlag(1, 0);	    // auto reset//kevin
    runTaskStack = aqStackInit(RUN_TASK_SIZE, "RUN");

    //runData.runTask = CoCreateTask(runTaskCode, (void *)0, RUN_PRIORITY, &runTaskStack[RUN_TASK_SIZE-1], RUN_TASK_SIZE);//kevin

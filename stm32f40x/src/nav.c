...
int nCounter  __attribute__((section(".ccm")));//5ms conter   
float chPitchPer __attribute__((section(".ccm")));
float chRollPer __attribute__((section(".ccm")));
float Pitchf __attribute__((section(".ccm")));
float Rollf __attribute__((section(".ccm")));
float deltaPitch __attribute__((section(".ccm")));

#endif

#ifdef _VSPD_SLOW
float chThroPer __attribute__((section(".ccm")));
float Throf __attribute__((section(".ccm")));
#endif
...
void navSetHfReference(uint8_t refType) {
	...
	navData.hfReferenceCos = arm_cos_f32(navData.hfReferenceYaw);//kevin5 cosf
	navData.hfReferenceSin = arm_sin_f32(navData.hfReferenceYaw);//kevin5 sinf
    }
}
...
void navSetFixType(void) {
    ...
    float vAccMsk = gpsData.vAcc + runData.accMask;//kevin
    
    if ((posTimeDlta < NAV_MAX_GPS_AGE) && (hAccMsk < NAV_MIN_GPS_ACC) && (vAccMsk < NAV_MIN_GPS_VACC) && (gpsData.fix_type >= 3))
        navData.fixType = 3;
    else if ((posTimeDlta < NAV_MAX_FIX_AGE) && (hAccMsk < NAV_MIN_FIX_ACC) && (vAccMsk < NAV_MIN_FIX_VACC) && (gpsData.fix_type >= 2))
        navData.fixType = 2;
    else
        navData.fixType = 0;
}
...
void navNavigate(void) {
    ...
    //a=get_mag_declination(22.528806f, 113.939888f);//254.152 //似乎不准啊
...
#ifdef _HSPD_SLOW
       Pitchf = Pitchf*0.9f + ((float)(RADIO_PITCH))*0.1f;
       Rollf = Rollf*0.9f + ((float)(RADIO_ROLL))*0.1f;  

        nCounter++;

        if (nCounter%5 == 0)
        {//25ms
            tmp = (chPitchPer - Pitchf);
            ...
            //deltaPitch = RADIO_ROLL;

        }
        navData.ChPitchFilter = navData.ChPitchFilter*0.9f + ((float)(chPitchPer))*0.1f;
        navData.ChRollFilter = navData.ChRollFilter*0.9f + ((float)(chRollPer))*0.1f;
#endif 
#ifdef _VSPD_SLOW

       Throf = Throf*0.9f + ((float)(RADIO_THROT))*0.1f;
        ...
        navData.ChThroFilter = chThroPer;//navData.ChThroFilter*0.9f + ((float)(chThroPer))*0.1f;
#endif
    ...
    // do we want to be in position hold mode?
        ...
        #ifdef _HSPD_SLOW
	if (reqFlightMode == NAV_STATUS_POSHOLD && (fabsf(navData.ChPitchFilter) > (int)p[CTRL_DEAD_BAND] || fabsf(navData.ChRollFilter) > (int)p[CTRL_DEAD_BAND]))
	...
	if (navData.mode == NAV_STATUS_ALTHOLD)
        ...
        navSetHoldAlt(ALTITUDE, 0);
	    //AQ_NOTICE("in manual mode.");//kevin
    }
    ...
    if (navData.mode == NAV_STATUS_DVH) {
        ...
        float y = 0.0f;
#ifdef _HSPD_SLOW
        if (navData.ChPitchFilter > p[CTRL_DEAD_BAND])
            x = -(navData.ChPitchFilter - p[CTRL_DEAD_BAND]) * factor;
        if (navData.ChPitchFilter < -p[CTRL_DEAD_BAND])
            x = -(navData.ChPitchFilter + p[CTRL_DEAD_BAND]) * factor;
        if (navData.ChRollFilter > p[CTRL_DEAD_BAND])
            y = +(navData.ChRollFilter - p[CTRL_DEAD_BAND]) * factor;
    ...
    // orbit POI
        ...
        // distance => velocity
#ifdef _HSPD_SLOW
        navData.holdSpeedNFilter = pidUpdate(navData.distanceNPID, 0.0f, UKF_POSN);
        navData.holdSpeedEFilter = pidUpdate(navData.distanceEPID, 0.0f, UKF_POSE);

        // normalize N/E speed requests to fit below max nav speed
        tmp = __sqrtf(navData.holdSpeedNFilter*navData.holdSpeedNFilter + navData.holdSpeedEFilter*navData.holdSpeedEFilter);
        if (tmp > navData.holdMaxHorizSpeed)
        {
            navData.holdSpeedNFilter = (navData.holdSpeedNFilter / tmp) * navData.holdMaxHorizSpeed;
            navData.holdSpeedEFilter = (navData.holdSpeedEFilter / tmp) * navData.holdMaxHorizSpeed;
        }

        if (fabsf(UKF_POSN)>4.0f)
        {//kevin 距离比较远时水平速度需要慢加     回家或者        任务模式到另一个点
            //加时缓一些
            if (fabsf(navData.holdSpeedNFilter) > fabsf(UKF_VELN))
            {
                navData.holdSpeedN += (navData.holdSpeedNFilter - navData.holdSpeedN) * 0.009f;
            }
            else
            {
                navData.holdSpeedN = navData.holdSpeedNFilter;
            } 
        }
        else
        {
        ...
        if (fabsf(UKF_POSE)>4.0f)
        {
            if (fabsf(navData.holdSpeedEFilter) > fabsf(UKF_VELE))
            {
                 navData.holdSpeedE += (navData.holdSpeedEFilter - navData.holdSpeedE) * 0.009f;
            }
            else
            {
                 navData.holdSpeedE = navData.holdSpeedEFilter;
            }
        }
        else
        {
    ...
    if (navData.mode > NAV_STATUS_MANUAL)
    {
        ...
        vertStick = navData.ChThroFilter - RADIO_MID_THROTTLE;
        if ((vertStick > p[CTRL_DBAND_THRO]  && !navData.setCeilingReached)  || vertStick < -p[CTRL_DBAND_THRO]) {
            ...
            navData.holdSpeedAlt += (navData.targetHoldSpeedAlt - navData.holdSpeedAlt) * 0.05f;//0.02
        ...
        // coming out of vertical override?
            ...
            navData.targetHoldSpeedAlt = constrainFloat(navData.targetHoldSpeedAlt, tmp, navData.holdMaxVertSpeed);
            // smooth vertical velocity changes
            navData.holdSpeedAlt += (navData.targetHoldSpeedAlt - navData.holdSpeedAlt) * 0.05f;//0.01
        }
        // PID has the throttle
            ...
            navData.targetHoldSpeedAlt = constrainFloat(navData.targetHoldSpeedAlt, tmp, navData.holdMaxVertSpeed);
            // smooth vertical velocity changes
            if(fabsf(ALTITUDE - navData.holdAlt) > 2.00f)//
            {//kevin 高度差2.5米以上：即返航或r    misson模式下从一个点到另一个点时       速度慢慢加上去        
               if(fabsf(navData.targetHoldSpeedAlt) > fabsf(VELOCITYD))//
               {//加速时缓加
                   navData.holdSpeedAlt += (navData.targetHoldSpeedAlt - navData.holdSpeedAlt) * 0.007f;
               }
               else
               {
    ...
    if (navData.mode == NAV_STATUS_MISSION) {
        ...
        if (curLeg->type == NAV_LEG_LAND && motorsData.throttle <= 1)
        {
            ...
            supervisorDisarm();
            navData.spvrModeOverride = 0;//kevin 否则lland后 再解锁在低油门时电机不会转，给人没解锁的感觉
        }
    }

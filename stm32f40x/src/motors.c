...
#endif
                *motorsData.pwm[i]->ccr = (supervisorData.state & STATE_ARMED) ? p[MOT_ARM] : p[MOT_ARM];// p[MOT_ARM]:0  kevin
        }
        // CAN
        else if (motorsData.can[i]) {
//	    motorsCheckCanStatus(i);
...
//   int i;
    ...
    // group disarm
//    for (i = 0; i < motorsData.numGroups; i++)
//	canCommandDisarm(CAN_TT_GROUP, i+1);//kevin
}
...
//    motorsSetCanGroup();
    motorsOff();
//    canTelemRegister(motorsReceiveTelem, CAN_TYPE_ESC);//kevin

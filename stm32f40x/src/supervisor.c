...
#if 1
rt_uint32_t *supervisorTaskStack;
...
void supervisorArm(void) {
    ...
    else if (/*rcIsSwitchActive(NAV_CTRL_AH) || rcIsSwitchActive(NAV_CTRL_PH) ||*/ rcIsSwitchActive(NAV_CTRL_MISN))//kevin
...
void supervisorCheckPendingActionRequest() {
    ...
    case SPVR_ACT_REQ_CFG_READ_FILE:
        //configLoadParamsFromFile();//kevin
        break;

    case SPVR_ACT_REQ_CFG_WRITE_FILE:
        //configSaveParamsToFile();//kevin
        break;
...
static void setLowBattery1Misson(void)
{
    ...
    if (navData.navCapable)
    {
        ...
        if (fsOption > SPVR_OPT_FS_RAD_ST2_LAND && navCalcDistance(gpsData.lat, gpsData.lon, navData.homeLeg.targetLat, navData.homeLeg.targetLon) > SUPERVISOR_HOME_POS_DETECT_RADIUS) {
            ...
            wp->maxVertSpeed = navData.homeLeg.maxVertSpeed;
            wp->poiHeading = navData.homeLeg.poiHeading;
            wp->loiterTime = 2*1000;//0 kevin
            wp->poiAltitude = 0.0f;

            wp = navGetWaypoint(wpi++);
            /* kevin 不要先降到到家的高度，直接一次性降到底
            // decend to home
...
static void setLowBattery2Misson(void)
{
...
static void setAutoTakeOffMisson(void)
{
    ...
    if (navData.navCapable)
    {
        ...
        wp->relativeAlt = 1;
        wp->targetAlt = 1.5f;
        wp->maxVertSpeed = 0.8f;
        wp->maxHorizSpeed = 0.0f;
        wp->poiHeading = AQ_YAW;
        wp->loiterTime = 0;
        wp->targetLat = gpsData.lat;
        wp->targetLon = gpsData.lon;
        wp->targetRadius = 1.0f;
        wp->poiAltitude = 0.0f;
...
static void setAutoLandMisson(void)
{
...
void supervisorTaskCode(void *unused) {
    ...
    volatile uint32_t swt_chg_count = 0;
    volatile int16_t swt_pre = 0;
    volatile uint32_t timeOverLimit = 0;
    volatile uint32_t take_off_delay = 0;
    volatile uint32_t land_delay = 0;
#endif
    ...
    //else if (canSensorsData.values[CAN_SENSORS_PDB_BATA] > 0.0f)//kevin
    ...
    while (1) {
        ...
        if ((supervisorData.state & STATE_DISARMED) && !(supervisorData.state & STATE_CALIBRATION)) {
            ...
            if (RADIO_THROT < p[CTRL_MIN_THROT] && RADIO_RUDD < -500/* && navData.mode == NAV_STATUS_MANUAL*/)//kevin
            {
                if (!supervisorData.armTime) {
                    supervisorData.armTime = timerMicros();
                }
                    ...
                    navData.spvrModeOverride = 0;//kevin
                    supervisorDisarm();
                    supervisorData.armTime = 0;
                }
            }
        ...
        {
            if (abs(radioData.channels[7]-swt_pre)>400)
            {
                if(!swt_chg_count)
                {
                    timeOverLimit = AUTO_SWT_TIME_OVER;
                }

                if ((timeOverLimit) && (!take_off_delay) && (!land_delay))
                    swt_chg_count++;
                ...
                if ((swt_chg_count >= AUTO_SWT_TIMES) && (timeOverLimit))
                {
                    ...
                    {   //设置起飞任务
                        if ((!take_off_delay) && (!land_delay))
                        {
                            ...
                            take_off_delay = 100;        //5s
                            AQ_NOTICE("Warning: set auto take off misson \n");
                        }
                    }
                    else
                    {   //设置降落任务
                        if  ((!take_off_delay) && (!land_delay))
                        {
                            land_delay = 100;        //5s
                            setAutoLandMisson();
                            AQ_NOTICE("Warning: set auto land misson \n");
                        }
                    }
                    swt_chg_count = 0;
                    timeOverLimit = 0;
...
void supervisorInit(void) {
    ...
    //supervisorData.supervisorTask = CoCreateTask(supervisorTaskCode, (void *)0, SUPERVISOR_PRIORITY, &supervisorTaskStack[SUPERVISOR_STACK_SIZE-1], SUPERVISOR_STACK_SIZE);//kevin
    rt_thread_init(&supervisorTask,
                   "radio",
                   supervisorTaskCode,
                   RT_NULL,
                   (rt_uint8_t*)supervisorTaskStack,
                   SUPERVISOR_STACK_SIZE*4,
                   SUPERVISOR_PRIORITY,
                   5);

    rt_thread_startup(&supervisorTask);
}
#endif
...
extern  volatile float  bg,sm,md;
...
void loggerSetup(void) {
	...
	switch (loggerFields[i].fieldId) {
		...
		break;
	    case LOG_GPS_HDOP:
		loggerData.fp[i].fieldPointer = (void *)&gpsData.hDOP;//ratep_mb;//deltaPitch;//;//navData.holdSpeedN;//gpsData.hDOP;
		break;
	    case LOG_GPS_VDOP:
		loggerData.fp[i].fieldPointer = (void *)&gpsData.vDOP;//ratep_dq;//navData.ChPitchFilter;//ratep_dq;//navData.holdSpeedE;//gpsData.vDOP;
		break;
	    case LOG_GPS_TDOP:
		...
		break;
	    case LOG_GPS_HACC:
		loggerData.fp[i].fieldPointer = (void *)&gpsData.hAcc;//agp_mb;//a;//gpsData.hAcc;//a kevin
		break;
	    case LOG_GPS_VACC:
		loggerData.fp[i].fieldPointer = (void *)&gpsData.vAcc;//agp_dq;//b;//gpsData.vAcc;//
		break;
		...
		//loggerData.fp[i].fieldPointer = (void *)&gimbalData.triggerLogVal;//kevin
		break;
	    case LOG_ACC_BIAS_X:
		...
		//loggerData.fp[i].fieldPointer = (void *)&canSensorsData.values[CAN_SENSORS_PDB_BATA];//kevin
		break;
	    case LOG_CURRENT_EXT:
		loggerData.fp[i].fieldPointer = (void *)&analogData.extAmp;
		break;
	    case LOG_VIN_PDB:
		//loggerData.fp[i].fieldPointer = (void *)&canSensorsData.values[CAN_SENSORS_PDB_BATV];//kevin
		break;
	}

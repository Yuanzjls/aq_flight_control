...
static void ubloxSetMode(void) {
    ...
    ubloxWriteU1(0x07);//kevin 6                 // /**< 0 Portable, 2 Stationary, 3 Pedestrian, 4 Automotive, 5 Sea, 6 Airborne <1g, 7 Airborne <2g, 8 Airborne <4g */
...
static void ubloxPollVersionNoDelay(void) {
    ubloxSendPreamble();
...
static void ubloxVersionSpecific(int ver) {
    ...
    AQ_PRINTF("gps hw num: %u \n", ubloxData.hwVer);//kevin

    if (ver > 7) {
        // 5Hz for ver 8 using multiple GNSS
        ubloxSetRate((uint16_t)100);//ubloxSetRate((uint16_t)200); //kevin
    }
    else if (ver > 6) {
        // 10Hz for ver 7
...
void ubloxSendSetup(void) {
    ...
    ubloxEnableMessage(UBLOX_TIM_CLASS, UBX_ID_NAV_SOL, 1);     //kevin for fix_type and sta num
    ubloxEnableMessage(UBLOX_NAV_CLASS, UBX_ID_NAV_PVT, 1);     //kevin for fix_type and sta num
...
unsigned char ubloxPublish(void) {
    ...
    //CoSetPriority(gpsTaskData.gpsTask, 1);//kevin
    int priority = 1;
    ...
    else if (ubloxData.class == UBLOX_NAV_CLASS && ubloxData.id == UBX_ID_NAV_PVT) {
    ...
    else if (ubloxData.class == UBLOX_NAV_CLASS && ubloxData.id == UBX_ID_NAV_SOL) {
	...
	rt_thread_control(&gpsTask,RT_THREAD_CTRL_CHANGE_PRIORITY,&priority);	
    //CoSetPriority(gpsTaskData.gpsTask, GPS_PRIORITY);//kevin

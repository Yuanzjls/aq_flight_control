...
void adcTaskCode(void *unused) {
    ...
    while (1) {
	// wait for work
	//CoWaitForSingleFlag(adcData.adcFlag, 0);//kevin
     rt_event_recv(adcData.adcFlag, (1 << 0), \
...
void adcInit(void) {
	...
	adcData.adcFlag = rt_event_create("adc", 0, RT_IPC_FLAG_FIFO);
...
void ADC_DMA_HANDLER(void) {
    ...
    *s   += (w[35]);				// temp3
	...
	rt_event_send(adcData.adcFlag, (1 << 0));//isr_SetFlag(adcData.adcFlag);

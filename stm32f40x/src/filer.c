...
static struct rt_thread filerTask;
filerStruct_t filerData;
unsigned int *filerTaskStack;
...
static void filerProcessRequest(filerFileStruct_t *f) {
	...
	//CoSetFlag(f->completeFlag);//kevin
    rt_sem_release(&f->completeFlag);
    }
}
...
void filerTaskCode(void *p) {
	...
	rt_thread_detach(&filerTask);//CoExitTask();//kevin
	return;
    }
	...
	if (filerData.files[i].allocated) {
	    ...
	    //CoSetFlag(filerData.files[i].completeFlag);//kevin
		rt_sem_release(&filerData.files[i].completeFlag);
	}
    }
    ...
    int i=0;
	rt_sem_control(&filerData.filerFlag,RT_IPC_CMD_RESET,&i);//CoClearFlag(filerData.filerFlag); //kevin
	rt_sem_take(&filerData.filerFlag, 5);// kevin CoWaitForSingleFlag(filerData.filerFlag, 5);	// run at least every 5ms if possible
    }
}
...
void filerInit(void) {
    ...
    //filerData.filerFlag = CoCreateFlag(0, 0);   // manual reset//kevin
	rt_sem_init(&filerData.filerFlag, "filef", 0, RT_IPC_FLAG_FIFO);
	
    filerTaskStack = aqStackInit(FILER_STACK_SIZE, "FILER");

    //filerData.filerTask = CoCreateTask(filerTaskCode, (void *)0, FILER_PRIORITY, &filerTaskStack[FILER_STACK_SIZE-1], FILER_STACK_SIZE);//kevin
     rt_thread_init(&filerTask,
                            "filer",
                            filerTaskCode,
                            RT_NULL,
                            (rt_uint8_t*)filerTaskStack,
                            FILER_STACK_SIZE*4,
                            FILER_PRIORITY,
                            5);
 
    rt_thread_startup(&filerTask);
...
int8_t filerGetHandle(char *fileName) {
	...
	if (!filerData.files[i].allocated || !(strcmp(filerData.files[i].fileName, fileName))) {
	    ...
	    //filerData.files[i].completeFlag = CoCreateFlag(0, 0);//kevin
         rt_sem_init(&(filerData.files[i].completeFlag), "i", 0, RT_IPC_FLAG_FIFO);
	    return i;
	}
    }
...
int32_t filerReadWrite(filerFileStruct_t *f, void *buf, int32_t seek, uint32_t length, uint8_t function) {
    ...
    rt_sem_release(&filerData.filerFlag);//CoSetFlag(filerData.filerFlag);
	int i=0;
    rt_sem_control(&f->completeFlag,RT_IPC_CMD_RESET,&i);//CoClearFlag(f->completeFlag);//kevin
    rt_sem_take(&f->completeFlag, 0);//CoWaitForSingleFlag(f->completeFlag, 0);//kevin

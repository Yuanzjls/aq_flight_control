...
//#define COMM_LOG_FNAME		"MSG"		// comment out to disable logging//kevin
...
#define AQ_NOTICE		commNotice //kevin
...
typedef struct {
    ...
    //OS_TID commTask;//kevin
    //OS_MutexID txBufferMutex;
    //OS_EventID notices;
		
	struct rt_mutex txBufferMutex;
	struct rt_mailbox notices;

} __attribute__ ((aligned (8))) commStruct_t;//kevin

//commStruct_t __attribute__((aligned));
extern commStruct_t commData;
extern struct rt_thread  commTask;

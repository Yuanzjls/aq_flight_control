...
#ifdef  _ADD_PRES_OFT	
  float pres_alt_offset;
  #define PRES_OFT_INT -100.0f
#endif
...
float navUkfPresToAlt(float pressure) {
    ...
    tmp = (1.0f - powf(pressure / UKF_P0, 0.1902632f          )) * 44330.1711145f;
    if (pres_alt_offset == PRES_OFT_INT)
   	{
...
//	if (isnan(q[0])||isnan(q[0])||isnan(q[0])||isnan(q[0]))
//		test=1;
    qr[0] *= norm;
    qr[1] *= norm;
    qr[2] *= norm;
...
void navUkfFinish(void) {
    ...
    //    x' = x cos f - y sin f
    //    y' = y cos f + x sin f
    navUkfData.yawCos = arm_cos_f32(navUkfData.yaw * DEG_TO_RAD);//cosf //kevin3
    navUkfData.yawSin = arm_sin_f32(navUkfData.yaw * DEG_TO_RAD);//sinf
}
...
int optical_flow=0;
int16_t fw_x,fw_y,fw_qua;//kevin
float fw_grd;
...
void navUkfInit(void) {
    ...
    mag[0] = arm_cos_f32(p[IMU_MAG_INCL] * DEG_TO_RAD);//cosf kevin5
    mag[1] = 0.0f;
    mag[2] = -arm_sin_f32(p[IMU_MAG_INCL] * DEG_TO_RAD);//sinf kevin5

    // rotate local mag vector to align with true north
    navUkfData.v0m[0] = mag[0] * arm_cos_f32(p[IMU_MAG_DECL] * DEG_TO_RAD) - mag[1] * sinf(p[IMU_MAG_DECL]  * DEG_TO_RAD);//kevin 5 cosf
    navUkfData.v0m[1] = mag[1] * arm_cos_f32(p[IMU_MAG_DECL] * DEG_TO_RAD) + mag[0] * sinf(p[IMU_MAG_DECL]  * DEG_TO_RAD);//kevin 5 cosf
    ...
    navUkfData.flowRotCos = arm_cos_f32(UKF_FLOW_ROT * DEG_TO_RAD);//kevin5 cosf
    navUkfData.flowRotSin = arm_sin_f32(UKF_FLOW_ROT * DEG_TO_RAD);//kevin5 sinf

...
static void dIMUTaskCode(void *unused) {
    ...
    while (1) {
	// wait for work  2.5ms
	//CoWaitForSingleFlag(dImuData.flag, 0);//kevin
...
void dIMUInit(void) {
    ...
    //task = CoCreateTask(dIMUTaskCode, (void *)0, DIMU_PRIORITY, &dIMUTaskStack[DIMU_STACK_SIZE-1], DIMU_STACK_SIZE);//kevin
     rt_thread_init(&task,
                            "dimu",
                            dIMUTaskCode,
                            RT_NULL,
...
void DIMU_ISR(void) {
	...
	rt_interrupt_enter();//CoEnterISR();//kevin
	rt_event_send(dImuData.flag, (1 << 0));//isr_SetFlag(dImuData.flag);
	rt_interrupt_leave();//CoExitISR();		
    }

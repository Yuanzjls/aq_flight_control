...
void srcdkfMeasurementUpdate(srcdkf_t *f, float32_t *u, float32_t *ym, int M, int N, float32_t *noise, SRCDKFMeasurementUpdate_t *measurementUpdate) {
	...
	int i, j;
	 ...
	 S = f->S;				// number of states
	 Xa = f->Xa.pData;			// sigma points
	 xIn = f->xIn;			// callback buffer
	 xNoise = f->xNoise;		// callback buffer
	 xOut = f->xOut;			// callback buffer
	 Y = f->Y.pData;			// measurements from sigma points
	 y = f->y.pData;			// measurement estimate
	 Sn = f->Sn.pData;			// observation noise covariance
	 qrTempM = f->qrTempM.pData;
	 C1 = f->C1.pData;
	 C1T = f->C1T.pData;
	 C2 = f->C2.pData;
	 D = f->D.pData;
	 inov = f->inov.pData;		// M x 1 matrix
	 xUpdate = f->xUpdate.pData;	// S x 1 matrix
	 x = f->x.pData;			// state estimate
	 Sx = f->Sx.pData;
	 Q = f->Q.pData;
	 qrFinal = f->qrFinal.pData;	

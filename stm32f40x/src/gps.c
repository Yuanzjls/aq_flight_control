...
void gpsCheckBaud(serialPort_t *s) {
    if ((IMU_LASTUPD - gpsData.lastMessage) > 5000000)//10000000 kevin
    {
...
void gpsTaskCode(void *p) {
	...
	while (serialAvailable(s)) {
		...
		//CoSetFlag(gpsTaskData.gpsPosFlag);//kevin
		  rt_sem_release(gpsTaskData.gpsPosFlag);	
	    }
	    // velocity update
	    else if (ret == 2) {
		// notify world of new data
		//CoSetFlag(gpsTaskData.gpsVelFlag);//kevin
...
void gpsInit(void) {
    ...
    //gpsTaskData.gpsVelFlag = CoCreateFlag(0, 0);//kevin
    //gpsTaskData.gpsPosFlag = CoCreateFlag(0, 0);
    gpsTaskStack = aqStackInit(GPS_STACK_SIZE, "GPS");

    //gpsTaskData.gpsTask = CoCreateTask(gpsTaskCode, (void *)0, GPS_PRIORITY, &gpsTaskStack[GPS_STACK_SIZE-1], GPS_STACK_SIZE);//kevin
...
void gpsSetEnabled(bool enable) {
    ...
    else
	{
		...
		rt_thread_control(&gpsTask,RT_THREAD_CTRL_CHANGE_PRIORITY,&priority);		
	//CoSetPriority(gpsTaskData.gpsTask, CFG_LOWEST_PRIO);	//kevin
	}	

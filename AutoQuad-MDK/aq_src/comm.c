...
void commNotice(const char *s) {
    ...
    if (commData.noticeQueueInit)
        rt_mb_send(&commData.notices, (rt_uint32_t )s);
}
...
commTxBuf_t *commGetTxBuf(uint8_t streamType, uint16_t maxSize) {
    ...
    if (commData.typesUsed & streamType) {
        ...
        rt_mutex_take(&commData.txBufferMutex, RT_WAITING_FOREVER);
        ...
        rt_mutex_release(&commData.txBufferMutex);
...
static void commCheckNotices(void) {
    ...
    if(rt_mb_recv(&commData.notices,(rt_uint32_t*)&s, RT_WAITING_NO)== RT_EOK)
		{
        int i;
#ifdef COMM_LOG_FNAME
        // write to disk
...
void commInit(void) {
    ...
    // setup mutex
    rt_mutex_init(&commData.txBufferMutex, "smutex1", RT_IPC_FLAG_FIFO);

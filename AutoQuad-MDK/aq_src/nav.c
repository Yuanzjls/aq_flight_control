...
void navSetHfReference(uint8_t refType) {
	...
	navData.hfReferenceCos = arm_cos_f32(navData.hfReferenceYaw);// cosf
	navData.hfReferenceSin = arm_sin_f32(navData.hfReferenceYaw);// sinf
    }
}
...
void navSetFixType(void) {
    ...
    float vAccMsk = gpsData.vAcc + runData.accMask;
...
void navNavigate(void) {
    ...
    if (navData.mode > NAV_STATUS_MANUAL)
    {
        ...
        // PID has the throttle
            ...
            {//高度差2.5米以上：即返航或r    misson模式下从一个点到另一个点时       速度慢慢加上去        
    ...
    if (navData.mode == NAV_STATUS_MISSION) {
        ...
        if (curLeg->type == NAV_LEG_LAND && motorsData.throttle <= 1)
        {
            ...
            supervisorDisarm();
            navData.spvrModeOverride = 0;// 否则land后 再解锁在低油门时电机不会转，给人没解锁的感觉
        }
    }

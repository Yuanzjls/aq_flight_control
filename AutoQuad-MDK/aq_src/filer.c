...
void filerTaskCode(void *p) {
    ...
    int i=0;
	rt_sem_control(&filerData.filerFlag,RT_IPC_CMD_RESET,&i);
	rt_sem_take(&filerData.filerFlag, 5);	// run at least every 5ms if possible
    }
}
...
int32_t filerReadWrite(filerFileStruct_t *f, void *buf, int32_t seek, uint32_t length, uint8_t function) {
    ...
    rt_sem_release(&filerData.filerFlag);
	int i=0;
    rt_sem_control(&f->completeFlag,RT_IPC_CMD_RESET,&i);
    rt_sem_take(&f->completeFlag, 0);
